...
@patch("src.users.application.services_handlers.UserServiceHandler.register_user", new_callable=AsyncMock)
def test_register_user_success(mock_register_user, client):
    mock_register_user.return_value = {"id": 1, "name": "Test User", "email": "test@example.com"}
    user_data = {"name": "Test User", "email": "test@example.com", "password": "securepassword123"}
...
def test_register_user_failure(mock_register_user, client):
    ...
    assert response.json() == {"detail": "An error occurred while registering the user."}


@patch("src.users.application.services_handlers.UserServiceHandler.get_user_by_id", new_callable=AsyncMock)
def test_get_user_by_id_success(mock_get_user_by_id, client):
    mock_get_user_by_id.return_value = {"id": 1, "name": "Test User", "email": "test@example.com"}
